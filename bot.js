...
const prefix = "-";
const adminprefix = "-";
const devs = ['423893025949876224'];
client.on('ready', () => {
  client.user.setGame('Emprie Server','https://www.twitch.tv/Empire');
  console.log('---------------');
...
client.on("messaصصge", async message => {
  if(!message.channel.guild) return;
  var prefix = "$";
...
client.on('message', message => {                      
    if(!message.channel.guild) return;
       ...
       let active = msg.createReactionCollector(activeFilter, { time: 15000 });
                               ...
                               active.on("collect", r => {
                                   message.member.addRole(message.guild.roles.find("name", "online"));
